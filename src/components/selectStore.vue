    ...
    <div class="selectStore popupSelect">
        <mt-popup v-model="popupVisible" position="right" class="modelRight">
            ...
            <section v-infinite-scroll="loadMore" infinite-scroll-disabled="isMore" infinite-scroll-distance="10">
                <ul class="dataList">
                    <li v-for="(item,index) in popupArr" :key="index" @click="currentObj = item" :class="[currentObj.WHID == item.WHID ? 'active' : '']">
                        {{item.WHName}}
                        <span class="icon" v-if="currentObj.WHID == item.WHID"><i class="iconfont icon-hook"></i></span>
                    </li>
                </ul>
...
export default {
    ...
    methods: {
        ...
        open() {
            ...
            this.$http.get('/api/SelectWarehouse', { params: {fid: this.account.fid} }).then(res => {
